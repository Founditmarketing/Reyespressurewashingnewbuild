  ...
  return (
    <section className="py-24 bg-slate-950 overflow-hidden relative">
      ...
      <div className="relative flex overflow-hidden group">
        <div className="animate-marquee flex gap-8 py-4" style={{ width: 'max-content' }}>
          {/* Strip 1 */}
          {testimonials.map((t, i) => (
            <div key={`a-${i}`} className="w-[340px] md:w-[440px] flex-shrink-0 p-8 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 shadow-2xl transition-transform hover:-translate-y-2">
          ...
          {testimonials.map((t, i) => (
            <div key={`b-${i}`} className="w-[340px] md:w-[440px] flex-shrink-0 p-8 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 shadow-2xl transition-transform hover:-translate-y-2">
              <div className="flex gap-1 mb-6">
                {[...Array(t.rating)].map((_, idx) => (
                  <svg key={idx} className="w-5 h-5 text-aqua" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                ))}
        ...
        {/* Gradient fades */}
        <div className="absolute top-0 bottom-0 left-0 w-16 md:w-32 bg-gradient-to-r from-slate-950 to-transparent pointer-events-none z-10" />
        <div className="absolute top-0 bottom-0 right-0 w-16 md:w-32 bg-gradient-to-l from-slate-950 to-transparent pointer-events-none z-10" />
      </div>
    </section>
  );
};
